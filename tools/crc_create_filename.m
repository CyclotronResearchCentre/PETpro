function [filename, pth] = crc_create_filename(p, file)
  % Old file name managment scriopt from bids-matalb
  % saved here for compatibility reasons
  %
  ...
  % USAGE::
  %
  % [filename, pth] = bids.create_filename(p, file)
  %
  ...
  for iEntity = 1:numel(entities)
    ...
    if ~isempty(required_entities) && ...
            strcmp(this_entity, required_entities) && ...
